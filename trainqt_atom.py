...
def graph_train():   
    if method == "motif":
        ...
        motif = False
    datamodule = AtomDataModule(train_path=train_path, val_path=val_path, test_path=test_path, tokenizer=tokenizer, batch_size=256, seed=seed, motif=motif)
    datamodule.setup()
    train_loader = datamodule.train_dataloader()
    val_loader = datamodule.val_dataloader()
    test_loader = datamodule.test_dataloader()
    logger = pl.loggers.TensorBoardLogger(save_dir="tb_logs", name=f"3.30-{method}-{seed}-qt")
    trainer = Trainer(
